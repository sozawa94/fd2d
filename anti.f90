program main
  !$ use omp_lib
  implicit none
  !2D anti-plane full-dynamic BIEM with slip-weakening friction law.
  ...
  real(8)::tmp1,tmp2,t,tp,tr,tau0,et,x,y,r,sin2,cos2,kern31,kern32,up,ur,ang
...
!stop
  ...
  allocate(kernt(0:kmax,imax,imax_),V(0:kmax,imax),D(imax),summt(imax_),summn(imax_),S0(imax),N0(imax),summtg(imax),summng(imax),rupt(imax))
  ...
  do i=1,imax_
    do j=1,imax
      ...
      do k=0,kmax
        ...
        !write(25,'(3i6,4e15.6)') i,j,k,kernt(i,j,k)
      end do
      !write(25,*)
    end do
    !write(25,*)
  ...
  do k=1,kmax
    write(25,'(3i6,e15.6)') i,j,k,kernt(k,j,i)
  end do
  end if
  !stop
  ! do i=1,imax
  ...
  do k=1,kmax
    ...
    do i=1,imax
      ...
      if(state(i).eq.1) then
        ...
        if(D(i).le.fdc) then
          V(k,i)=1/(1+factor)*(-2.d0*cs/mu*(tr+(tp-tr)*(1.d0-(D(i)+0.5d0*dt*V(k-1,i))/fdc)-S0(i))-summtg(i))
          V(k,i)=0.5d0*(V(k-1,i)+V(k,i))
        else
          V(k,i)=-2d0*cs/mu*(tr-S0(i))-summtg(i)
        end if
    else if(S0(i)-mu/2.d0/cs*summtg(i).gt.tp) then
      state(i)=1
      rupt(i)=k*dt
      V(k,i)=-2.d0*cs/mu*(max(tr,tr+(tp-tr)*(1.d0-D(i)/fdc))-S0(i))-summtg(i)
    else
      V(k,i)=0.d0
    end if
...
subroutine initialcrack(hypox,hypoy,xcol,ycol,S0,imax,dx)
  ...
  !gaussian
   ...
   !crack
  !  do i=0,imax
  !    kerns(i)=2d0*mu/3.d0/pi*(1.d0/(dx*(i-0.5d0))-1.d0/(dx*(i+0.5d0)))
  !  end do
  !  do i=1,imax
  !    !xcol=dx*(i-imax/2-0.5d0)
  !    rr=(xcol(i)-hypox)**2+(ycol(i)-hypoy)**2
  !    if(rr.lt.lc**2) then
  !      D(i)=0.45d0*sqrt(lc**2-rr)
  !    end if
  !  end do
  !  do i=1,imax
  !    do j=1,imax
  !      S0(i)=S0(i)+kerns(abs(i-j))*D(j)
  !    end do
  !    write(*,*) S0(i),D(i)
  !  end do
